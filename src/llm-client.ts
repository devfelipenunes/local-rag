...
export function applyRateLimits(
  limits: Record<string, { size: number; window: number }>,
): void {
...
setInterval(() => {
  for (const q of _queues.values()) q.trim();
}, 10_000).unref();
...
async function _fetchWithLogging(
  url: string,
  options: RequestInit,
  model: string,
): Promise<Response> {
  ...
  try {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      try {
        ...
        const retryable =
          isNetwork ||
          (httpStatus !== undefined && RETRYABLE_STATUS.has(httpStatus));
        if (!retryable) throw err;
        const hinted =
          err instanceof LlmHttpError ? err.retryAfterMs : undefined;
        const backoffMs = Math.min(
          hinted ?? Math.min(2 ** i, 60) * 1000,
          MAX_RETRY_AFTER_MS,
        );
  ...
  return _withRetry(
    () => {
      ...
      switch (spec.provider) {
        ...
        default:
          return _callOllamaSimple(prompt, spec.model, baseUrl, timeout);
      }
    },
    spec.model,
    attempts,
...
async function _callOllamaSimple(
  prompt: string,
  model: string,
  baseUrl: string,
  timeout: number,
): Promise<string> {
  const resp = await _fetchWithLogging(
...
async function _callOpenAISimple(
  prompt: string,
  model: string,
  apiKey: string,
  baseUrl: string,
...
async function _callAnthropicSimple(
  prompt: string,
  model: string,
  apiKey: string,
  baseUrl: string,
...
async function _callGeminiSimple(
  prompt: string,
  model: string,
  apiKey: string,
  baseUrl: string,
...
async function _callOllamaWithTools(
  ...
  const resp1 = await _fetchWithLogging(
    `${baseUrl}/api/chat`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const resp2 = await _fetchWithLogging(
    `${baseUrl}/api/chat`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function _callOpenAIWithTools(
  ...
  try {
    ...
    throw new Error(
      `OpenAI tool call: invalid JSON in arguments: ${tc.function.arguments}`,
    );
  }
  let toolResult: string;
  try {
  ...
  return _withRetry(
    () => {
      ...
      switch (spec.provider) {
        ...
        default:
          return _callOllamaTool(prompt, tool, spec.model, baseUrl, timeout);
      }
    },
    spec.model,
    attempts,
...
async function _callOllamaTool(
  prompt: string,
  tool: ToolDef,
  model: string,
  baseUrl: string,
...
async function _callOpenAITool(
  prompt: string,
  tool: ToolDef,
  model: string,
  apiKey: string,
...
async function _callAnthropicTool(
  prompt: string,
  tool: ToolDef,
  model: string,
  apiKey: string,
...
async function _callGeminiTool(
  prompt: string,
  tool: ToolDef,
  model: string,
  apiKey: string,
