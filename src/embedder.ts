  ...
  for (const text of texts) {
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      ...
      if (!resp.ok) {
        ...
        return Promise.reject(
          new Error(
            `Embed failed: ${resp.status} ${resp.statusText} — ${body}`,
          ),
        );
      }
    ...
    if (!pushed) {
      return Promise.reject(
        new Error("Embed failed: max retries exceeded (rate limit)"),
      );
    }
  }
...
function embedBatchAttempt(
  ...
  return providerFn(texts, baseUrl, timeout).catch((err: unknown) => {
    ...
    return new Promise<void>((r) => setTimeout(r, (attempt + 1) * 1000)).then(
      () => embedBatchAttempt(texts, attempt + 1),
    );
  });
}

// Resilient embedding: some providers (Ollama /api/embed with certain models)
// ignore `truncate:true` and reject inputs that exceed their REAL context length
// (often far below the advertised value). One oversized/pathological text must not
// abort a whole batch — bisect on failure; for a single failing text, shrink it
// until the provider accepts, so every record still gets a vector.
async function embedBatchResilient(
  texts: string[],
  depth: number,
): Promise<number[][]> {
  if (texts.length === 0) return [];
  try {
    return await embedBatchAttempt(texts, 0);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    // Only bisect on provider input-length errors; other errors propagate.
    if (
      !/input length exceeds|context length|too large|maximum length|400/.test(
        msg,
      )
    )
      throw err;
    if (texts.length === 1) {
      const t = texts[0];
      let budget = Math.floor(cfg.embedMaxChars / 2);
      while (budget >= 32) {
        try {
          const vecs = await embedBatchAttempt([t.slice(0, budget)], 0);
          process.stderr.write(
            `[embedder] shrink text ${t.length}→${budget} chars (depth ${depth})\n`,
          );
          return vecs;
        } catch (_e) {
      ...
      return embedBatchAttempt([t.slice(0, 32)], 0);
    }
    const mid = Math.ceil(texts.length / 2);
    const left = await embedBatchResilient(texts.slice(0, mid), depth + 1);
    const right = await embedBatchResilient(texts.slice(mid), depth + 1);
    return [...left, ...right];
  }
}

export async function embedBatch(texts: string[]): Promise<number[][]> {
  const maxChars = cfg.embedMaxChars ?? 3000;
  const truncated = texts.map((t) =>
    t.length > maxChars ? t.slice(0, maxChars) : t,
  );
  return embedBatchResilient(truncated, 0);
}
...
export function generateDescription(chunk: {
  ...
  const prompt =
    ...
    `"${chunk.name}" does:\\n\\n${preview}`;
  return callLlm(prompt, 200).then((text) => text.trim().slice(0, 3000));
}
...
export function llmFilter(
  ...
  return callLlm(prompt, 256)
    .then((text) => {
      ...
      if (!m) {
        process.stderr.write(
          `[embedder] llmFilter: no JSON array in response — keeping all\\n`,
        );
        return candidates;
      }
      ...
      return indices
        .filter(
          (i): i is number =>
            typeof i === "number" && i >= 0 && i < candidates.length,
        )
        .map((i) => candidates[i]!);
    })
    .catch((err: unknown) => {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(
        `[embedder] llmFilter ${msg} — returning unfiltered\\n`,
      );
      return candidates;
    });
}
